...
function getImage($image, $size = null, $avatar = false, $cover = false)
{
    $clean = '';
...
function formatProfileViewCount($count)
{
    if ($count >= 1000000) {
...
{
    $sellerFixedCharge = gs('seller_fixed_charge');
    $sellerPercentCharge = gs('seller_percent_charge');

    return $sellerFixedCharge + ($price * $sellerPercentCharge / 100);
}
...
function displayRating(float $val)
{
    $result = '';
    $fullStars = floor($val);
    $fraction = $val - $fullStars;
    $halfStar = $fraction >= 0.25 && $fraction < 0.75;
