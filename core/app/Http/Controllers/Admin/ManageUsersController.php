...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $widget['charge']      = OrderDetails::where('seller_id', $user->id)->whereHas('order', function($query) {
            $query->where('status', Status::ORDER_COMPLETED);
        })->sum('charge');

        $widget['purchase']    = Order::where('status', Status::ORDER_COMPLETED)->where('user_id',$user->id)->sum('total');
        $widget['approved_prompts']        = Prompt::approved()->where('user_id',$user->id)->count();
        $widget['rejected_prompts']      = Prompt::rejected()->where('user_id',$user->id)->count();

        $promptsCount = Prompt::where('user_id', $user->id)->count();
        $sellCount = OrderDetails::where('seller_id', $user->id)->count();
        ...
        return view('admin.users.detail', compact('pageTitle', 'user','widget','totalDeposit','totalWithdrawals','totalTransaction','countries','sellCount','promptsCount'));
    }
