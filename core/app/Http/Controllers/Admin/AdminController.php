...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $prompt['total']       = Prompt::where('step', '>=', 3)->count();
        $prompt['pending']     = Prompt::where('step', '>=', 3)->pending()->count();
        $prompt['approved']    = Prompt::approved()->count();
        $prompt['featured']    = Prompt::approved()->featured()->count();


        $order['total']       = Order::count();
        $order['pending']     = Order::pending()->count();
        $order['completed']   = Order::completed()->count();
        $order['canceled']    = Order::canceled()->count();

        $count['charge']      = OrderDetails::whereHas('order', function($query) {
            $query->where('status', Status::ORDER_COMPLETED);
        })->sum('charge');

        $count['category']    = Category::active()->count();
        $count['tool']        = Tool::active()->count();
        $count['coupon']      = Coupon::active()->count();
        ...
        return view('admin.dashboard', compact('pageTitle', 'prompt', 'order','count', 'widget', 'chart','deposit','withdrawals'));
    }
