@php
    $promptContent = getContent('featured_prompts.content', true);
    ...
    $showSeeMore = $prompts->count() > 6;
    $prompts = $prompts->take(6);
@endphp
            ...
            <div class="section-heading style-left d-flex flex-wrap justify-content-between gap-2">
                ...
                @if($showSeeMore)
                    <div class="section-heading__right">
                        <a href="{{ route('prompt.featured') }}" class="btn btn-outline--base">@lang('View all')</a>
                    </div>
                @endif
            </div>
            <div class="row gy-4">
                @include($activeTemplate . 'partials.related_prompt', [
                    'prompts' => $prompts,
                ])
            </div>
        </div>
