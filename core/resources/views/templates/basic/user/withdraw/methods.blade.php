    ...
    <form action="{{ route('user.withdraw.money') }}" method="post" class="withdraw-form">
        ...
        <div class="gateway-card custom--card">
            <div class="row justify-content-center gy-sm-4 gy-3">
                <div class="card-header pt-0 d-flex flex-wrap justify-content-between align-items-center">
                    <h6 class="mb-0 py-2">{{ __($pageTitle) }}</h6>
                    <a class="btn btn--base btn--sm" href="{{ route('user.withdraw.history') }}"> <i class="las la-wallet"></i>
                        ...
                        @foreach ($withdrawMethod as $data)
                            <label for="{{ titleToKey($data->name) }}" class="payment-item @if ($loop->index > 4) d-none @endif gateway-option">
                                ...
                                <div class="payment-item__thumb">
                                    <img class="payment-item__thumb-img" src="{{ getImage(getFilePath('withdrawMethod') . '/' . $data->image) }}" alt="payment-thumb">
                                </div>
