        ...
        <div class="trending-item-inner">
            <div class="trending-item">
                ...
                <div class="trending-item__thumb">
                    <a href="{{ route('prompt.details', $prompt->slug) }}"><img src="{{ getImage(getFilePath('prompt') . '/' . 'thumb_' . @$prompt->image) }}" alt="image"></a>
                </div>
                <div class="trending-item__content">
                    <div class="trending-item__content-top">
                        <div class="left">
                            ...
                            <a href="{{ route('seller.profile', $prompt->user->username) }}" class="username">{{ '@' . $prompt->user->username }} <img src="{{ asset($activeTemplateTrue . 'images/thumbs/badge.svg') }}" alt="img"></a>
                        </div>
                        <span class="price text--gradient">{{ gs('cur_sym') }}{{ showAmount($prompt->price, currencyFormat: false) }}</span>
                    </div>

                    @auth
                        <a class="btn btn-outline--base w-100 mt-3 btn-add-to-cart" data-prompt-id="{{ $prompt->id }}">@lang('Add to cart')</a>
                    @else
